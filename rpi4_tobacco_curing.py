...
def save_state():
    ...
    state = {
        ...
        "stage_start_temp": stage_start_temp,
    }
...
def load_state():
    ...
    global current_mode, current_stage_index, auto_target_temp, stage_start_time, stage_start_temp
    try:
        with open(STATE_FILE, "r") as f:
            state = json.load(f)
            current_mode = state.get("current_mode", current_mode)
            current_stage_index = state.get("current_stage_index", current_stage_index)
            auto_target_temp = state.get("auto_target_temp", auto_target_temp)
            stage_start_time = state.get("stage_start_time", stage_start_time)
            stage_start_temp = state.get("stage_start_temp", stage_start_temp)
...
def show_splash_screen():
    ...
    lcd.clear()
    lcd.write_string("Tobacco Curing System")
    lcd.crlf()
...
def main():
    ...
    lcd.clear()
    lcd.write_string("Initialization OK")
    time.sleep(2)
